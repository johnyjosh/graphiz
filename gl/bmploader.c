...
// Compression
typedef enum {BI_RGB=0, BI_RLE8, BI_RLE4 }CompressionType ;
...
static LOAD_TEXTUREBMP_RESULT loadBMP(const char* filename, 
                               unsigned char** bitmapData)
{
  ...
  *bitmapData = NULL; 
  ...
  str = (char*)malloc((strlen(filename)+5)*sizeof(char));
  if (!str)
  ...
  if (!res)
  {
    ...
    *bitmapData = (unsigned char*)malloc(width*height*3*sizeof(char));
...
LOAD_TEXTUREBMP_RESULT load2DTextureBMP(const char* filename, 
                                              GLuint *textureName,
                                              GLint internalformat)
{
  ...
  char* str = (char*)malloc((strlen(filename)+5)*sizeof(char));
...
//  glPushClientAttrib(GL_CLIENT_PIXEL_STORE_BIT);
  ...
  if (!res)
  {
    ...
    //glTexImage2D(GL_TEXTURE_2D,0, internalformat, width, height,0, GL_RGB, GL_UNSIGNED_BYTE, &bitmapData);		      
  }
